...
class GBList:
    ...
    def init_granular_balls(self, purity=1):
        ...
        while True:
            ...
            if self.granular_balls[i].purity < purity:
                ...
                radie = np.mean(dis)  # Calculate the average distance within the ball from the center of the ball
...
def funtion(ball_list):
    ...
    for i in range(len(Ball_list)):
        ...
        Ball_list[i].radius = np.mean(ball_dis)  # Take the average distance as the radius of the sphere
        ...
        if len(ball) == 0:
            ...
            for index, values in enumerate(ball):
                ...
                if values[2] != Ball_list[j].label and (np.sum((values[0] - Ball_list[j].center) ** 2) ** 0.5) <= (
                        values[1] + Ball_list[j].radius):  flag = True
                ...
                if len(split_clusters) == 1:
                    ...
                    Ball_list[j].radius = np.mean(ball_dis)
                    ...
                    for new_ball in split_clusters[1:]:
                        ...
                        new_ball.radius = np.mean(ball_dis)
...
def nearest_knn(ball_list, test):
    # Test nearest neighbor accuracy
    num = []
...
def main():
    ...
    # keys = ['fourclass_0.1', 'votes_0.1', 'breastcancer_0.1', 'mushroom_0.1', 'svmguide1_0.1', 'svmguide3_0.1']
    ...
    for d in range(len(keys)):
        ...
        for i in range(10):  # Repeat the experiment 10 times
            ...
            for train_index, test_index in skf.split(train_data, train_target):
                ...
                a, ballnum, testtime = fun(train, test)
                avg, std = mean_std(a)
                Acc += avg  # Total accuracy
                bnum += ballnum  # Total number of granular balls
                times += testtime
                # Std += std
            print(Acc / 10, bnum / 10, times / 10)
            Acc = Acc / 10
            bnum = round(bnum / 10, 0)
            times = round(times * 100, 0)
            info.append([Acc, bnum, times])
        info = pd.DataFrame(info)
        info = info.values
        # The row of data with the highest accuracy in the 10 experiments: highest accuracy, number of granular balls, classification time (ms)
        print("result: ", info[np.where(info[:, 0] == np.max(info[:, 0]))[0], :])
